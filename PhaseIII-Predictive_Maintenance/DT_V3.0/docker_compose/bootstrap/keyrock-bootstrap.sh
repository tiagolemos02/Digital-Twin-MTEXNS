...
ensure_permission_for_role "$ADMIN_ROLE_ID" "Historical Data — Read QuantumLeap" \
  "Reading historical telemetry from QuantumLeap" \
  "GET" '^/quantumleap/v2/.*' true
...
ensure_viewer_orion_working_hours_rule() {
  ROLE_ID="$1"
  PNAME="${2:-Orion Logs — Read entities}"
  PDESC="${3:-Viewer can read Orion logs only in Lisbon working hours}"
  PACTION="GET"
  PRES="${4:-^/v2/entities/?(\?.*)?$}"
  PREGEX=true
...
<Rule RuleId="$PID" Effect="Permit">
  <Description>$PDESC (Mon-Fri 09:00-13:00 and 14:00-18:00)</Description>
  <Target>
    <AnyOf>
      <AllOf>
        <Match MatchId="urn:oasis:names:tc:xacml:1.0:function:string-regexp-match">
          <AttributeValue DataType="http://www.w3.org/2001/XMLSchema#string">$PRES</AttributeValue>
...
ensure_viewer_orion_working_hours_rule "$VIEWER_ROLE_ID" \
  "Historical Data — Read QuantumLeap entities" \
  "Viewer can read historical telemetry only in Lisbon working hours" \
  '^/quantumleap/v2/entities/.*'
